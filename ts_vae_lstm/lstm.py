...
class TSLSTMDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = torch.as_tensor(x_subset).to(torch.float32)
        y = torch.as_tensor(y_subset).unsqueeze(0).to(torch.float32)
        x_emb, y_emb = self.get_embeddings(x), self.get_embeddings(y)
        x_emb = self.standardize(x_emb).to(torch.float32)
        y_emb = self.standardize(y_emb).to(torch.float32)
